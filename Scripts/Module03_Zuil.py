...
bericht_display = f"SELECT bericht FROM review inner join moderatie m on m.mod_id = review.mod_id WHERE goedkeuring LIKE 'y' ORDER BY datum DESC"
curs_bericht.execute(bericht_display)
inlezen_berichten = curs_bericht.fetchall()

curs_naam = conn.cursor()
naam_display = f"SELECT naam FROM review inner join moderatie m on m.mod_id = review.mod_id WHERE goedkeuring LIKE 'y' ORDER BY datum DESC"
curs_naam.execute(naam_display)
inlezen_naam = curs_naam.fetchall()

curs_infobericht = conn.cursor()
infobericht_display = f"SELECT datum, station FROM review ORDER BY datum DESC"
...
def infobericht_vernieuwen(teller3):
    try:
        vardatumbericht.set(
            datetime.strftime((datetime.strptime(str(inlezen_infobericht[teller3][0]), '%Y-%m-%d')),
                              '%Y-%m-%d'))
    except IndexError:
        pass
    if teller3 < 4:
